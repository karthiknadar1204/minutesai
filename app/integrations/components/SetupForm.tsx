    ...
    const handleSubmit = () => {
        if (createNew) {
            ...
            const selected = items?.find((item: any) =>
                item.id === selectedId || item.key === selectedId || item.gid === selectedId
            )
            onSubmit(platform, {
                [`${itemLabel}Id`]: selected?.id || selectedId,
                [`${itemLabel}Name`]: selected?.name || selectedName,
                projectKey: selected?.key || selectedId,
                workspaceId: data?.workspaceId
            })
        }
